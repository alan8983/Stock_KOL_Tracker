...
/// 情緒標記繪製器
/// 負責在正確位置繪製情緒標記（五角形）
class SentimentMarkersPainter extends CustomPainter {
  final FlChartController controller;
  final ChartThemeConfig theme;

  SentimentMarkersPainter({
  ...
  void paint(Canvas canvas, Size size) {
    if (controller.prices.isEmpty || controller.posts.isEmpty) return;

    for (final post in controller.posts) {
      // 1. 找到對應的 K 線索引（順延邏輯）
      final index = _findCandleIndex(post.postedAt);
  ...
  void _drawDashedGuideLine(
    ...
    if (candleIndex < 0 || candleIndex >= controller.prices.length) return;

    final price = controller.prices[candleIndex];
    final endY = pointsDown
        ? controller.priceToY(price.high) // Bearish: 連接到 high
        : controller.priceToY(price.low); // Bullish: 連接到 low
  ...
  @override
  bool shouldRepaint(covariant SentimentMarkersPainter oldDelegate) {
