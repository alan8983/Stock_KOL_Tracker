...
1. **多標的分析**：為每個提及的美股代號獨立判斷情緒
   ...
   - 為每個標的獨立判斷情緒：Bullish (看多), Bearish (看空), Neutral (中立)
   - 判斷哪個標的是「主要標的」（文章主要討論的標的，通常篇幅最多或論述最深入）
   - 如果只有一個標的，自動設為主要標的
2. 辨識 KOL 名稱（通常在文章開頭1-3行或結尾，如「蕭上農」、「IEObserve 國際經濟觀察」等）
3. 辨識發文時間（支援相對時間如「3小時前」、「16小時」，或絕對時間如「12月11日下午2:02」）
4. 提供核心論述摘要（3-5個要點，每點30字內）
   - 專注於投資論述、市場觀點、技術分析等內容
   - 避免重複 metadata（股票代號、情緒判斷等）
   - 用精簡、專業的語言呈現
5. 識別文章頭尾的冗餘文字（metadata 資訊）
...
注意：
- **tickerAnalyses 是必填欄位**，即使只有一個標的也要放在陣列中
- 每個 ticker 必須有獨立的 sentiment 判斷，不要假設所有標的都是相同情緒
- isPrimary 只能有一個為 true，通常是文章主要討論的標的
